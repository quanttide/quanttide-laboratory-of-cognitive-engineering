...
基于 task 1.3 的意图关系图，将 13 个意图按层级和领域组织为结构化的领域模型——即"意图逐步包裹为完整想法"。

**输入**：`outputs/task-1.3-intent-relationship.md`（13 个意图 + 关系图 + 元意图分析 + 冲突分析）

**方法**：
1. **分层**：按抽象程度将所有意图分为 3 层
   - **基础层（Foundation）**：支撑一切的前提性意图（心理可持续、基础设施）
   - **方法层（Methodology）**：定义"怎么做"的方法论意图（POC、意图工程、标准化）
   - **应用层（Application）**：具体领域的实践意图（写作云、商业模式、团队机制、交互范式）
2. **组合**：识别跨层的意图组合——哪些方法意图服务于哪些应用意图，形成"想法束"
3. **映射**：将每束"想法"映射回原始日志的具体日期和关键段落，验证结构化结果的完整性
4. **命名**：为每个组合（完整想法）赋予名称，使其可被后续任务直接引用

**输出**：`outputs/task-1.4-structured-model.md`（已执行），包含：
- 领域模型总览图（分层结构 + 跨层组合）
- 每层详细说明（层内意图、层间关系）
- 完整想法列表（每个想法 = 意图组合 + 名称 + 原始证据映射）
