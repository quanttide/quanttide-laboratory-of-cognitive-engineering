...
引擎将 YAML 文件中的结构视为一个静态知识图谱：

```
[心理可持续性(id:2)] ──支持──→ [研发方法论(id:1)]
       ↑                          │
       │                          ↓
[商业模式(id:5)] ←──验证/驱动── [认知工程(id:3)]
       │                          │
       │                          ↓
       └──────冲突────→ [心理可持续性(id:2)]
```

每条边都带有类型（支持/冲突/依赖）和逻辑说明。
...
**输入**：一段新文本（如新的日志条目、一个观点陈述、一个待分析的想法）

**推理流程**：
1. **意图匹配** — 将输入文本与 10 个 Intent 簇进行语义匹配，输出匹配度排序
2. **关系推演** — 根据匹配到的 Intent 簇，从 `intent-relation.yaml` 中提取其关联意图及关系类型
...
**测试 A：单意图匹配**
- 输入一段仅涉及单一 Intent 簇的文本（如仅谈心理状态）
- 预期：匹配到 1 个 Intent，推理路径短

**测试 B：多意图冲突**
- 输入一段同时涉及商业压力和研发探索的文本
...
**执行方法**：
以 `assets/refined/intent.yaml` 和 `intent-relation.yaml` 为上下文，将测试输入逐条提交给 AI 进行推理，记录推理过程和输出。
...
**输出**：`outputs/evaluation.md`，包含每组的推理结果记录和改进建议。
